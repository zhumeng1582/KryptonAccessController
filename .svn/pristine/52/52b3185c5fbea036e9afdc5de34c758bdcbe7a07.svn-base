...
namespace KryptonAccessController.AccessSdkDataBase.DAL
{
	...
	public partial class Manager
	{
		...
		public bool Add(KryptonAccessController.AccessSdkDataBase.Model.Manager model)
		{
			...
			strSql.Append("ManagerID,ManagerName,ManagerPassWord)");
		...
		public KryptonAccessController.AccessSdkDataBase.Model.Manager GetModel(int ManagerID,string ManagerName)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 ManagerID,ManagerName,ManagerPassWord from Manager ");
			strSql.Append(" where ManagerID=@ManagerID and ManagerName=@ManagerName ");
			SqlParameter[] parameters = {
					new SqlParameter("@ManagerID", SqlDbType.Int,4),
					new SqlParameter("@ManagerName", SqlDbType.NVarChar,50)			};
			...
			KryptonAccessController.AccessSdkDataBase.Model.Manager model=new KryptonAccessController.AccessSdkDataBase.Model.Manager();
		...
		/// 得到一个对象实体
		/// </summary>
		public KryptonAccessController.AccessSdkDataBase.Model.Manager DataRowToModel(DataRow row)
		{
			KryptonAccessController.AccessSdkDataBase.Model.Manager model=new KryptonAccessController.AccessSdkDataBase.Model.Manager();
			if (row != null)
			{
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select ManagerID,ManagerName,ManagerPassWord ");
        ...
        public KryptonAccessController.AccessSdkDataBase.Model.Manager GetModel(KryptonAccessController.AccessSdkDataBase.Model.Manager model)
        {
            ...
            strSql.Append(" and ManagerPassWord=@ManagerPassWord ");
            SqlParameter[] parameters = {
					new SqlParameter("@ManagerName", SqlDbType.NVarChar,50),
					new SqlParameter("@ManagerPassWord", SqlDbType.NVarChar,20)};
            ...
            KryptonAccessController.AccessSdkDataBase.Model.Manager newModel = new KryptonAccessController.AccessSdkDataBase.Model.Manager();
            DataSet ds = DbHelperSQL.Query(strSql.ToString(), parameters);
            if (ds.Tables[0].Rows.Count > 0)
            {
                return DataRowToModel(ds.Tables[0].Rows[0]);
